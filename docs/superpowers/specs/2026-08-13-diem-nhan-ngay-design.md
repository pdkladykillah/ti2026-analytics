...
Một ngày Swiss có **8 loạt**. Với cỡ mẫu đó, ranh giới giữa mô tả và suy luận chính là ranh
giới giữa dùng được và bịa:

| loại | ví dụ | với 8 loạt |
|---|---|---|
| mô tả | "LGD thắng Falcons 2–1" | đúng chắc chắn |
| đếm | "Muerta bị cấm 7/8 loạt" | đúng chắc chắn |
...
| nguồn | nội dung | nhịp |
|---|---|---|
| `ScheduledSeries` (Valve) | loạt nào xong, tỷ số loạt | **15 phút** |
| `Matches` + `MatchPlayers` + `DraftEvents` (OpenDota) | thời lượng, pick/ban, chỉ số | **6 giờ** |

Đo lúc 04:5x ngày 13/08: bảng đấu đã ghi 2/8 loạt xong, nhưng mới **4 ván** của giải nằm trong
...
Tự chốt khi **mọi loạt được xếp trong ngày đó đều `IsCompleted`**. Bộ làm tươi 15 phút đã biết
trạng thái từng loạt nên không cần thêm lịch riêng.
...
1. **Tổng quan** — số loạt xong/tổng, số ván đọc được/ước tính, trung vị thời lượng.
2. **Ngược kèo** — loạt mà bên thắng có winrate 180 ngày (từ `TeamStatSnapshot`) THẤP hơn bên
...
- **Loạt bị dời sang ngày khác.** Ngày lấy theo `ActualAt` khi có, chỉ dùng `ScheduledAt` khi
  chưa đá — vì hôm nay các trận đã trượt một tiếng so với giờ công bố.
...
3. Chốt đúng lúc loạt cuối xong, không sớm hơn.
4. Không có ván nào nạp được thì vẫn ra digest, và cờ độ phủ nói đúng.
5. Loạt dời ngày được tính vào ngày ĐÁ THẬT, không phải ngày xếp lịch.
